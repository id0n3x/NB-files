...
def updateMsgs(client, message,redis):
  ...
  if text and text == "نقل البيانات" and rank == "sudo":
    if redis.smembers(BOT_ID+"Tshake:Chek:Groups"):
      ...
      Bot("sendMessage",{"chat_id":chatID,"text":"تم ايجاد ({}) مجموعات خاصه بسورس تشاكي سيتم نقلها الى (<a href=\"http://t.me/calmapubg\">Loma</a>)\nقد يستغرق بعض الوقت".format(Ngp),"reply_to_message_id":message.message_id,"parse_mode":"html","disable_web_page_preview":True})
    else:
    ...
    Bot("sendMessage",{"chat_id":chatID,"text":"تم نقل البيانات من سورس تشاكي الى (<a href=\"http://t.me/calmapubg\">Loma</a>)","reply_to_message_id":message.message_id,"parse_mode":"html","disable_web_page_preview":True})
